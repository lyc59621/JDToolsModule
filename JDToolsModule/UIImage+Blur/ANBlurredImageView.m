...
-(void)generateBlurFramesWithCompletion:(void(^)())completion{
    ...
    NSMutableArray  *tempFrameArr = [[NSMutableArray alloc]init];
    NSMutableArray  *tempFramesReverseArr = [[NSMutableArray alloc]init];
    UIColor  *tintCOlor =_blurTintColor;
    ...
    for (int i = 0; i < frames; i++){
        dispatch_group_async(g, serialQueue, ^{
            UIImage *blurredImage = [downsampledImage drn_boxblurImageWithBlur:((CGFloat)i/frames)*blurLevel withTintColor:[tintCOlor colorWithAlphaComponent:(CGFloat)i/frames * CGColorGetAlpha(tintCOlor.CGColor)]];
            
            if (blurredImage){
                // Our normal animation.
                [tempFrameArr addObject:blurredImage];
                // Our reverse animation.
                [tempFramesReverseArr insertObject:blurredImage atIndex:0];
            }
        });
    }
    dispatch_group_notify(g, dispatch_get_main_queue(), ^{
        _framesArray = tempFrameArr;
        _framesReverseArray = tempFramesReverseArr;
        NSLog(@"模糊添加完成设置Image");
         completion();
    });
//    completion();
